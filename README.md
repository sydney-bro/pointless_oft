...
pointless was originally deployed as a standard ERC20 token on Polygon network.<br />
Later on, we decided to go multi-chain and deploy it on base and zkSync too. We used layerzero OFT (Omnichain fungible token) standard to deploy pointless on base and zkSync.<br />
Because we already had a token, we created an OFT Adapter on polygon. On base and zkSync, it is just plain OFT.<br />
Below are the pointless CAs:<br />
```bash
